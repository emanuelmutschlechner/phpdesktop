...
json_value* GetApplicationSettings() {
    ...
    PDLOG_DEBUG << "Fetching settings from settings.json file";

    std::string settingsFile = GetExecutableDirectory() + "\\settings.json";
    std::string contents = GetFileContents(settingsFile);
    if (contents.empty()) {
        PDLOG_WARNING << "Error while opening the settings.json file";
    ...
    json_value* json_parsed = json_parse_ex(&settings, contents.c_str(),
                                            &error[0]);
    if (json_parsed == 0) {
        PDLOG_WARNING << "Error while parsing settings.json file: " << error;
