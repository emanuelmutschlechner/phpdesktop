...
bool StartWebServer() {
    PDLOG_INFO << "Starting Mongoose " << mg_version() << " web server";
    ...
    PDLOG_INFO << "WWW directory: " << wwwDirectory;
    ...
    PDLOG_INFO << "Index files: " << indexFiles;
    ...
    PDLOG_INFO << "CGI interpreter: " << cgiInterpreter;
    ...
    if (cgiPattern.empty())
        cgiPattern = "**.php$";
    PDLOG_INFO << "CGI pattern: " << cgiPattern;
    ...
    PDLOG_INFO << "Hide files patterns: " << hide_files_patterns;
    ...
    if (!cgi_temp_dir.length() || !DirectoryExists(cgi_temp_dir)) {
        if (cgi_temp_dir.length()) {
            PDLOG_WARNING << "cgi_temp_dir directory does not exist: "
                        << cgi_temp_dir;
        }
    ...
    PDLOG_INFO << "CGI environment variables set: " << cgiEnvironment;
    ...
    PDLOG_INFO << "Web server url: " << g_webServerUrl;
...
void StopWebServer() {
    if (g_mongooseContext) {
        PDLOG_INFO << "Stopping Mongoose web server";
        /*
        ...
        */
        ...
        PDLOG_DEBUG << "Mongoose webserver stopped immediately";
    }
}
